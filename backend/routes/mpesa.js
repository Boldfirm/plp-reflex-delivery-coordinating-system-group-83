...
router.post('/mpesa/stk-push', async (req, res) => {
  try {
    ...
    if (!phone || !deliveryId) return res.status(400).json({ error: 'Phone number and deliveryId required' });
    const amount = Number(process.env.MPESA_AMOUNT) || 200;
    const checkoutRequestId = 'ws_CO_' + Date.now();
    const t = now();
    const d = jRead();
    if (!d.payments) d.payments = [];
    d.payments.push({ id: 'PAY-' + Date.now(), deliveryId, riderId: riderId || null, phone, amount, status: 'PENDING', checkoutRequestId, createdAt: t });
    jWrite(d);
...
router.get('/mpesa/status/:id', async (req, res) => {
  try {
    ...
    res.json({ paid: delivery?.paymentStatus === 'PAID', paymentStatus: delivery?.paymentStatus || 'UNPAID' });
